...
######################################################
def add_x6(output_file, n_detectors, rho, z_position, flip, ringN=0, energy_resolution=0):
    dphi = 360./n_detectors
    if energy_resolution>0:
        content_x6_resolution = f"""%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Resolution
STARK
    Type = X6
    Reso = {energy_resolution}
"""
        print(content_x6_resolution, file=output_file)
...
######################################################
def make_p_elastic(particle_name, energy, cs_file_name = "flat.txt", z_emission=-1000, sigma_energy=0, sigma_theta=0, sigma_x=0, sigma_y=0, ex_light=0, ex_heavy=0):
    reaction_conf_name = f"{particle_name}_elastic"
    if ex_heavy!=0: reaction_conf_name = f"{particle_name}_exh_{ex_heavy}"
...
TwoBodyReaction
    ...
    Light= 1H
    Heavy= {particle_name}
    ExcitationEnergyLight= {ex_light} MeV
    ExcitationEnergyHeavy= {ex_heavy} MeV
...
if __name__ == "__main__":
    ...
    add_x6    (output_file=det_file, n_detectors=12, rho= 95.0, z_position=131+0.5*75, flip=1, ringN=1, energy_resolution=2)
    add_x6    (output_file=det_file, n_detectors=12, rho=102.0, z_position=131+0.5*75, flip=0, ringN=1)
    add_x6    (output_file=det_file, n_detectors=16, rho=120.0, z_position= 45+0.5*75, flip=1, ringN=2)
    #cs_example = make_gaus_cs_file(90,15)
    #reaction, conf = make_p_elastic(particle_name="40Ar", energy=8*40, cs_file_name=cs_example)
    reaction, conf = make_p_elastic(particle_name="40Ar", energy=8*40, cs_file_name="data_cs/cross_section_e8.txt")
    make_write_script(name, det_file.name, reaction, conf, macro_for_sim="root draw_ko2421.C")
    ...
    #cs_example = make_gaus_cs_file(90,15)
    #reaction, conf = make_p_elastic(particle_name="40Ar", energy=8*40, cs_file_name=cs_example)
    reaction, conf = make_p_elastic(particle_name="40Ar", energy=8*40, ex_heavy=1.46, cs_file_name="data_cs/cross_section_1stex_e8.txt")
    make_write_script(name, det_file.name, reaction, conf, macro_for_sim="root draw_ko2421.C")
