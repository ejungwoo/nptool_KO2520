...
void FillGoodEvent(double energy, TVector3 pos, double weight);
double ThetaComToLab(double theta_com) { return (180. - theta_com)/2.; }
double ThetaLabToCom(double theta_lab) { return (180. - 2*theta_lab); }

void draw_ko2421(TString fileName1="", TString fileName2="")
{
    bool test = 0;
    Init();

    if (fileName1.IsNull()) {
        //fileName1 = "data/stark_ko2520_21Na.21Na_elastic.sim.root";
        fileName1 = "data/stark_ko2421.40Ar_elastic.sim.root"; fileName2 = "data/stark_ko2421_2.40Ar_exh_1.46.sim.root";
        //fileName1 = "data/stark_ko2421_2.40Ar_exh_1.46.sim.root"; fileName2 = "";
    }
    ...
    //TChain *tree = new TChain("SimulatedTree");
    //tree -> AddFile(fileName1);
    //if (fileName2.IsNull()==false) tree -> AddFile(fileName2);
    ...
    for (auto iFile : {0,1})
    {
        ...
        for (auto iEvent=0; iEvent<numEvents; ++iEvent)
        {
            ...
            if (goodEvent>=0)
                FillGoodEvent(energyGood, posGood, weight);
        }
        file -> Close();
    }
...
LKDrawingGroup *fTop;
TH2D *fHistTrack_AA[4];
TH1D *fHistTrack_TTCoM[4];
TH1D *fHistTrack_TTLab[4];
TH2D *fHistTrack_ETLab[4];
...
void Init()
{
    ...
    auto group4 = fTop -> CreateGroup("good_event_lab",true);
    ...
    for (auto i : {0,1})
    {
        fHistTrack_ETLab[i] = new TH2D(Form("fHistTrack_ETLab_%d",i),Form("track info (%s);theta_{Lab};e",TString((i==0?"proton":"all")).Data()),180,20,80,150,0,30);
        fHistTrack_TTCoM[i] = new TH1D(Form("fHistTrack_TTA_%d",i),Form("track info (%s);theta (deg);yield",TString((i==0?"proton":"all")).Data()),2*180, 0,180);
        fHistTrack_TTLab[i] = new TH1D(Form("fHistTrack_TTALab_%d",i),Form("track info (%s);theta (deg);yield",TString((i==0?"proton":"all")).Data()),2*180, 0,180);
        fHistTrack_AA[i]    = new TH2D(Form("fHistTrack_AA_%d",i),Form("track info (%s);theta (deg);phi (deg)",TString((i==0?"proton":"all")).Data()),180, 0,180,180,-180,180);

        group1 -> CreateDrawing() -> Add(fHistTrack_ETLab[i]);
        //group1 -> CreateDrawing() -> Add(fHistTrack_TTCoM[i]);
        //group1 -> CreateDrawing() -> Add(fHistTrack_TTLab[i]);
        //auto draw = group1 -> CreateDrawing();
        //draw -> Add(fHistTrack_AA[i]);
    ...
    fHistETCoM = new TH2D("fHistETCoM", "hit info;theta_{CM};e",180,0,180,150,0,30); drawET -> Add(fHistETCoM);
    fHistThCoM = new TH1D("fHistThCoM", "hit info;theta_{CM}",180,0,180); group3 -> CreateDrawing() -> Add(fHistThCoM);
    fHistETLab = new TH2D("fHistETLab", "hit info;theta_{Lab};e",180,20,80,150,0,30);
    fHistThLab = new TH1D("fHistThLab", "hit info;theta_{Lab}",180,20,80);

    auto drawETLab = group4 -> CreateDrawing();
    drawETLab -> SetStatCorner(0);
    drawETLab -> Add(fHistETLab);
    //group4 -> CreateDrawing() -> Add(fHistETCoM);
    group4 -> CreateDrawing() -> Add(fHistThLab);
...
{
    if (iTrack==2) {
        fHistTrack_ETLab[0] -> Fill(tta,ke);
        fHistTrack_TTCoM[0] -> Fill(ThetaLabToCom(tta));
        fHistTrack_TTLab[0] -> Fill(tta);
        fHistTrack_AA[0] -> Fill(ThetaLabToCom(tta),phi);
    }
    if (ThetaLabToCom(tta)<160) {
        fHistTrack_ETLab[1] -> Fill(tta,ke);
        fHistTrack_TTCoM[1] -> Fill(ThetaLabToCom(tta));
        fHistTrack_TTLab[1] -> Fill(tta);
        fHistTrack_AA[1] -> Fill(ThetaLabToCom(tta),phi);
    }
}
...
void FillGoodEvent(double energy, TVector3 pos, double weight)
{
    fHistETCoM -> Fill(ThetaLabToCom(pos.Theta()*180/pi),energy, weight);
    fHistThCoM -> Fill(ThetaLabToCom(pos.Theta()*180/pi));
    fHistETLab -> Fill(pos.Theta()*180/pi,energy, weight);
    fHistThLab -> Fill(pos.Theta()*180/pi);
}
